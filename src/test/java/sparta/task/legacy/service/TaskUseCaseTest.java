...
//        Mockito.when(this.taskJpaRepository.save(Mockito.any(Task.class)))
//                .thenReturn(task);
...
//        Mockito.when(this.userJpaRepository.findByUsername(Mockito.anyString()))
...
//        Mockito.when(this.taskJpaRepository.findById(Mockito.any(Long.class))).thenReturn(taskOptional);
...
//        // given
//        Mockito.when(this.taskJpaRepository.findById(Mockito.any(Long.class))).thenThrow(HttpStatusException.class);
//
//        // when & then
...
//        // given
//        Mockito.when(this.taskJpaRepository.findById(Mockito.any(Long.class))).thenReturn(Optional.of(task));
//        task.delete();
//
//        try {
//            // when
...
//        tasks.add(task);
//
//        Mockito.when(this.taskJpaRepository.findAllByDeletedAtIsNull(Sort.by(Sort.Direction.DESC, "createdAt")))
//                .thenReturn(tasks);
//
//        Mockito.when(this.taskMapper.toTaskDto(Mockito.any(Task.class)))
//                .thenReturn(taskResponseDto);
...
//
//        // when
//        // TaskResponseDto updated = this.taskService.updateTaskBy(1L, updateTaskRequestDto, currentUser);
//
//        // then
...
////        try {
////            // when
////            this.taskService.updateTaskBy(1L, wrongPasswordDto, currentUser);
////        } catch (HttpStatusException e) {
////            // then
...
//
//        Mockito.when(this.taskJpaRepository.findById(Mockito.any(Long.class)))
...
//        Mockito.verify(this.taskJpaRepository).save(mockTask);
//
//        // 실행된 횟수도 지정할 수 있음
